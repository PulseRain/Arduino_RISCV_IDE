...
class Reindeer_OCD:
    ...
    _OCD_DEBUG_TYPE_DRAM_READ_4_BYTES  = 0x6A
    ...
    _CONFIG_TYPE_DRAM_WRITE_4_BYTES_WITHOUT_ACK = 0x5E
    _CONFIG_TYPE_DRAM_WRITE_4_BYTES_WITH_ACK    = 0x5E | 1
    _CONFIG_TYPE_DRAM_WRITE_128_BYTES_WITH_ACK  = 0x59
    ...
    #  mem_read_32bit
    #========================================================================
    def mem_read_32bit (self, addr, pram0_dram1, show_crc_error=0):
        ...
        while (condition):
            ...
            if (pram0_dram1 == 0):
                ...
                frame_type_byte = Reindeer_OCD._OCD_DEBUG_TYPE_DRAM_READ_4_BYTES * 2 + Reindeer_OCD._toggle;
    ...
    def mem_read (self, addr, length, show_crc_error=0):
        ...
        pram0_dram1 = (addr >> 30) & 1
        
        for i in range (length // 4):
            ret = self.mem_read_32bit (addr, pram0_dram1)
    ...
    #  mem_write_32bit
    #========================================================================
    def mem_write_32bit (self, addr, data, pram0_dram1, ack=1, show_crc_error=0):
        addr_write_low_byte  = addr & 0xFF
        addr_write_high_byte = (addr >> 8) & 0xFF
        addr_write_high_high_byte = (addr >> 16) & 0xFF
        
        ##print ("wr32bit, addr = 0x%x  pram0_dram1 = %d" % (addr, pram0_dram1))
        ...
        while (condition):
            ...
            if (ack):
                if (pram0_dram1 == 0):
                    ...
                    frame_type_byte = Reindeer_OCD._CONFIG_TYPE_DRAM_WRITE_4_BYTES_WITH_ACK * 2 + Reindeer_OCD._toggle
            else:
                if (pram0_dram1 == 0):
                    frame_type_byte = Reindeer_OCD._CONFIG_TYPE_PRAM_WRITE_4_BYTES_WITHOUT_ACK * 2 + Reindeer_OCD._toggle
                else:
                    frame_type_byte = Reindeer_OCD._CONFIG_TYPE_DRAM_WRITE_4_BYTES_WITHOUT_ACK * 2 + Reindeer_OCD._toggle
    ...
    #  mem_write_128byte
    #========================================================================
    def mem_write_128byte (self, addr, data_list, pram0_dram1, is_text, show_crc_error=0):
        ...
        if ((pram0_dram1 == 0) and (self.text2data_length > 0) and (is_text != 0)):
            self.text2data_length = self.text2data_length - 1
            pram0_dram1 = 1
            print ("-------- move 128 bytes text to data, 0x%x" % addr)
        
        ##print ("wr128, addr = 0x%x , pram0_dram1 = %d, text2data_length = %d" % (addr, pram0_dram1, self.text2data_length))
        
        while (condition):
            if (pram0_dram1 == 0):
                frame_type_byte = Reindeer_OCD._CONFIG_TYPE_PRAM_WRITE_128_BYTES_WITH_ACK * 2 + Reindeer_OCD._toggle
            else:
                frame_type_byte = Reindeer_OCD._CONFIG_TYPE_DRAM_WRITE_128_BYTES_WITH_ACK * 2 + Reindeer_OCD._toggle
    ...
    def _write_mem (self, addr, data, pram0_dram1, is_text):
        offset = 0
        length = len (data)
        ...
        for i in range (total_128byte_frame):
            ...
            self.mem_write_128byte (addr + offset, data[offset : offset + 128], pram0_dram1, is_text)
            offset = offset + 128
        ...
        for i in range (total_words - total_128byte_frame * 32):
            ...
            self.mem_write_32bit(addr + offset, data_int, pram0_dram1)
            offset = offset + 4
    ...
    def load_elf (self, elf_file):
            ...
            if ('LOAD' in section_property_list[total_sections]):
                ...
                if ('CODE' in section_property_list[total_sections]):
                    ...
                    if (section_name.startswith("text")):
                        is_text_list.append(1)
                    else:
                        is_text_list.append(0)
                else:
                    pram0_dram1_list.append(1)
                    is_text_list.append(0)
        ...
        for i in range(total_sections):
            ...
            self._write_mem (addr_list[i], data_list_to_write, pram0_dram1_list[i], is_text_list[i])
    ...
    def load_hex (self, hex_file):
        ...
        for record in intel_hex_file.data_record_list:
            ...
            if (len(data_list_to_write) == 0):
                ...
                self._write_mem (addr, data_list_to_write_reorder, -1)
                data_list_to_write = record.data_list
                addr = record.address
...
if __name__ == "__main__":
    ...
    print ("===============================================================================")
    print ("# Copyright (c) 2021, PulseRain Technology LLC ")
    print ("# GRV3000D Configuration Utility, Version 2.3")
    ...
    try:
          opts, args = getopt.getopt(sys.argv[1:],"t:a:RrhP:b:i:d:l:c",["help", "run", "reset", "toolchain=", "port=", "start_addr=", "baud=", "image=", "dump_addr=", "dump_length=", "console_enable", "mem_test_len=", "text2data_len="])
    ...
    for opt, args in opts:
        if opt in ('-b', '--baud'): 
        ...
        elif opt in ('--text2data_len'):
            if (args.startswith("0x")):
                text2data_length = int (args, 16)
            else:
                text2data_length = int (args)
        else:
            print ("Usage:\n  ")
            print ("  Options: \n")
            print ("    -r, --reset          : reset the processor")
            ...
            print ("    --text2data_len=     : move code to data for the beginning part of .text section, in the multiple of 128 bytes.")
            print (" ")
            print ("    Example: To run the zephyr hello_world application")
            print ("     python GRV3000D_Config.py --port=COM9 --reset --image=C:\GitHub\Reindeer\bitstream_and_binary\zephyr\hello_world.elf --console_enable --run")
    ...
    try:
        ...
        ocd.objcopy = objcopy
        ocd.readelf = readelf
        ocd.text2data_length = text2data_length
